...
export default class TopicsContent extends Component {
    ...
    componentDidMount(){
        ...
        this.getThisTopic(topicId)
    }
    getThisTopic = async topicId =>{
        try {
            this.setState({title: ''})
    ...
    updateTitle = async (newTitle) => {
        try {
            ...
            const topicId = this.props.match.params.topicid
            if(newTitle !== ""){
                const topicToUpdate = await userTopics.doc(topicId)
                topicToUpdate.update({'topic.title': newTitle})
            }
        } catch (error) {
    ...
    updateContent = async (newContent) => {
        try {
            ...
            if(newContent !== ""){
                const topicToUpdate = await userTopics.doc(topicId)
                topicToUpdate.update({'topic.content': newContent})
            }
        } catch (error) {
